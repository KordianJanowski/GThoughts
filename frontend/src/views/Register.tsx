...
const Register:React.FC = () =>{
  ...
  const resizeFile = (file: Blob) => new Promise(resolve => {
    Resizer.imageFileResizer(file, 80, 80, 'JPEG/PNG/JPG', 100, 0,
    (uri) => {
      resolve(uri);
    }, 'base64' );
  });
  ...
  const {handleSubmit, handleChange, values, handleBlur, touched, errors} = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      email: Yup.string()
        ...
        .min(6, `${ isI18NisEnglish ? 'Email have to be longer than 6 characters' : 'Email musi być dłuższy niż 6 znaków' }`)
      ...
      password: Yup.string()
        ...
        .min(6, `${ isI18NisEnglish ? 'Password have to be longer than 6 characters' : 'Hasło musi być dłuższe niż 6 znaków'}`)
        .required(`${ isI18NisEnglish ? 'Required' : 'Pole wymagane' }`),
      repeatPassword: Yup.string()
        ...
        .min(6, `${ isI18NisEnglish ? 'Repeat password have to be longer than 6 characters' : 'Powtórzone hasło musi być dłuższe niż 6 znaków'}`)
